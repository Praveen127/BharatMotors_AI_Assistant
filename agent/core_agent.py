...
LLM BACKEND: real calls go through langchain_openai.ChatOpenAI when
USE_MOCK_LLM=false and an OPENAI_API_KEY + network are available. Otherwise
a deterministic MockLLM is used so the whole pipeline (including grounding
and safety behaviour, which do NOT depend on the LLM) is fully testable
offline. See docs/engineering_justification.md.
"""
import os
import re
import time
...
class RealLLM:
    """Wraps langchain_openai.ChatOpenAI for production use."""

...
agent/prompts.py + agent/memory.py + policy_rlhf.feedback_collector (style)
   |
   v
LLM (RealLLM via langchain_google_genai/Gemini, or MockLLM offline)
   |
   v
...
This whole pipeline is wrapped once, by `deployment/app.py` (a FastAPI
backend). The web UI, `deployment/streamlit_app.py`, is a **separate**
process that calls that API over HTTP — it never imports `SupportAgent`
directly. See §7 for why.
...
|---|---|---|
| LLM (`agent/core_agent.py`) | `langchain_google_genai.ChatGoogleGenerativeAI` (Gemini) | `MockLLM` — deterministic, but genuinely prompt-variant-sensitive (see `docs/prompt_comparison_table.md`) |
| Embeddings (`retrieval/embedder.py`) | `langchain_google_genai.GoogleGenerativeAIEmbeddings` (Gemini) | `OfflineEmbedder` — TF-IDF + truncated SVD (scikit-learn) |
...
| MCP tool server (`mcp/`) | `fastmcp` (`FastMCP`, stdio transport) | hand-rolled JSON-RPC 2.0 over stdio in `mcp/_stdio_fallback_server.py` (same `tools/list`/`tools/call` method contract fastmcp's stdio transport uses) |
...
`USE_MOCK_LLM=false` and `GOOGLE_API_KEY`. Every safety behaviour
...
these same tools over a real MCP server built on `fastmcp` (`mcp/server.py`)
rather than only as in-process Python calls means the tool layer could be
attached to any MCP-compatible client (Claude Desktop, a different agent
framework, an IDE assistant, etc.) without rewriting the tool logic — this
is the architectural benefit MCP is meant to provide, demonstrated with a
working (not just described) client/server round trip (see `mcp/client.py`
output in the repo's evidence logs, `logs/mcp_events.log`). `fastmcp` was
chosen over hand-writing the MCP wire protocol because its `@mcp.tool()`
decorator derives each tool's JSON schema directly from the existing
Python type hints and docstrings in `tools/tool_registry.py` — the tool
logic itself did not need to change to become MCP-servable.
...
- **Assumption:** Sessions and memory (`agent/memory.py`) are in-process and
  reset on restart. A production deployment behind the FastAPI backend
  (`deployment/app.py`) with multiple `uvicorn` workers/instances would
  need a shared session store (e.g. Redis) — noted but not built, since
  it's an infrastructure concern orthogonal to the agent logic (see
  `docs/deployment_guide.md`, "Known limitation: session state").
- **Design choice:** the web UI (`deployment/streamlit_app.py`) is a
  separate process from the API, communicating only over HTTP
  (`API_BASE_URL`) rather than importing `agent/core_agent.py` directly.
  This keeps the agent backend framework-agnostic — the same API could
  serve a different frontend (a mobile app, the MCP tool surface, another
  team's dashboard) without any change to `agent/`, `retrieval/`,
  `safety/`, or `tools/`.
...
- **Security:** `.env.example` documents `GOOGLE_API_KEY` as an environment
